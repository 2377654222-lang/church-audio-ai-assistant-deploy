...
def extract_features(path: str | Path) -> dict:
    ...
    low_energy = float(stft[(freqs >= 20) & (freqs < 250)].mean()) if stft.size else 0.0
    mid_energy = float(stft[(freqs >= 250) & (freqs < 4000)].mean()) if stft.size else 0.0
    high_energy = float(stft[(freqs >= 4000) & (freqs < 12000)].mean()) if stft.size else 0.0
    ...
    return {
        ...
        "spectrum": {
            ...
            "spectral_bandwidth_hz": clean_float(np.mean(spectral_bandwidth)),
